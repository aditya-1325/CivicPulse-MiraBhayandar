...
import { AlertCircle, X, Send, ExternalLink } from 'lucide-react';
...
/* ── Navbar ── */
function Navbar({ page, onNavigate, onReport }) {
  return (
    <nav className="glass-nav" style={{
      height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '0 48px', position: 'sticky', top: 0, zIndex: 40,
    }}>
      <button onClick={() => onNavigate('home')} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer' }}>
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#0ea5e9' }} className="animate-pulse-dot" />
        <span style={{ fontSize: 20, fontWeight: 700, color: '#0ea5e9', fontFamily: 'Inter, sans-serif' }}>CivicPulse</span>
        <span style={{ fontSize: 13, color: '#64748b', fontFamily: 'Inter, sans-serif' }}>Mira-Bhayandar</span>
      </button>
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)' }}>
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ duration: 0.2 }}
            className="glass" style={{ width: 480, maxHeight: '80vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              ...
              <button onClick={() => onClose(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={18} /></button>
            </div>
            ...
            <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <button onClick={() => onClose('submitted')} style={{ width: '100%', background: '#ef4444', color: 'white', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Submit Complaint</button>
            </div>
          </motion.div>
        </motion.div>
      )}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: "'Inter', sans-serif", position: 'relative' }}>
      <BackgroundOrbs />
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <Navbar page={page} onNavigate={setPage} onReport={() => setModalOpen(true)} />
        <div style={{ flex: 1, overflowY: page === 'dashboard' ? 'hidden' : 'auto' }}>
          <AnimatePresence mode="wait">
            {page === 'home' && <motion.div key="home" {...pageAnim}><HomePage onNavigate={setPage} /></motion.div>}
            {page === 'dashboard' && <motion.div key="dash" {...pageAnim} style={{ height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}><DashboardPage /></motion.div>}
            {page === 'track' && <motion.div key="track" {...pageAnim}><TrackPage /></motion.div>}
            {page === 'report' && <motion.div key="report" {...pageAnim}><ReportPage /></motion.div>}
            {page === 'survey' && <motion.div key="survey" {...pageAnim}><SurveyPage /></motion.div>}
          </AnimatePresence>
        </div>
      </div>
      ...
      <ReportModal open={modalOpen} onClose={handleModalClose} />
      <Toast {...toast} />
    </div>
  );
}
