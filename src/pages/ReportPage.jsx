...
export default function ReportPage({ isMobile }) {
  const m = isMobile;
      ...
      <AnimatePresence mode="wait">
        {submitted ? (
          <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}><CheckCircle2 size={32} color="#10b981" /></div>
            ...
            <button onClick={() => setSubmitted(false)} className="glass hover-lift" style={{ padding: '10px 24px', fontSize: 14, fontWeight: 600, color: '#0c1445', border: 'none', cursor: 'pointer', borderRadius: 12, fontFamily: 'Inter, sans-serif', minHeight: 48 }}>Submit Another</button>
          </motion.div>
        ) : (
          <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <form onSubmit={handleSubmit} className="glass" style={{ borderRadius: 24, padding: m ? '24px 20px' : 40 }}>
              ...
              <div style={{ marginBottom: 20 }}><label style={labelStyle}>Issue Type</label><select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })} style={inputStyle}>{['Pothole', 'Water Supply', 'Waste / Garbage', 'Streetlight', 'Drainage', 'Other'].map(t => <option key={t}>{t}</option>)}</select></div>
              <div style={{ marginBottom: 20 }}><label style={labelStyle}>Description</label><textarea value={formData.desc} onChange={e => setFormData({ ...formData, desc: e.target.value })} rows={3} placeholder="Describe the issue in detail..." style={{ ...inputStyle, minHeight: 100, resize: 'vertical' }} /></div>
              <div style={{ marginBottom: 20 }}><label style={labelStyle}>Location</label><input value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value })} style={inputStyle} /></div>
              ...
              <button type="submit" className="hover-lift" style={{ width: '100%', background: '#0ea5e9', color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', minHeight: 48 }}>Submit Complaint →</button>
            </form>
          </motion.div>
        )}
      </AnimatePresence>
