    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={{ paddingTop: 100, paddingBottom: 80, textAlign: 'center' }}>
        ...
        <motion.h1 initial="hidden" animate="visible" variants={stagger}
          style={{ fontSize: 64, fontWeight: 800, lineHeight: 1.1, letterSpacing: -2, color: '#0c1445', marginBottom: 24 }}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
          style={{ fontSize: 18, fontWeight: 400, color: '#64748b', maxWidth: 520, margin: '0 auto 40px', lineHeight: 1.7 }}>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
          ...
          {[
            ...
            <motion.div key={i} variants={fadeUp} custom={i} className="glass hover-lift"
              style={{ padding: '32px 24px', textAlign: 'center', borderRadius: 20 }}>
              <span style={{ fontSize: 48, fontWeight: 800, color: '#0ea5e9', display: 'block', marginBottom: 8 }}>
                <CountUp target={s.n} suffix={s.s || ''} />
              </span>
              <span style={{ fontSize: 13, color: '#64748b', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.l}</span>
            </motion.div>
          ))}
        </motion.div>
      </section>
      ...
      <section style={{ maxWidth: 1000, margin: '0 auto 80px', padding: '0 24px' }}>
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          style={{ fontSize: 32, fontWeight: 700, color: '#0c1445', textAlign: 'center', marginBottom: 48 }}>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
          ...
          {[
            ...
            <motion.div key={i} variants={fadeUp} custom={i} className="glass hover-lift"
              style={{ padding: '40px 32px', borderRadius: 24 }}>
      ...
      <section style={{ maxWidth: 1100, margin: '0 auto 80px', padding: '0 24px' }}>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
          ...
          {[
            ...
            <motion.div key={i} variants={fadeUp} custom={i} style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
              <div className="glass hover-lift" style={{ padding: '32px 24px', borderRadius: 20, textAlign: 'center', flex: 1 }}>
              ...
              {i < 3 && <ArrowRight size={20} color="rgba(14,165,233,0.3)" style={{ flexShrink: 0, margin: '0 4px' }} />}
            </motion.div>
          ))}
        </motion.div>
      </section>
      ...
      <section style={{ maxWidth: 800, margin: '0 auto 80px', padding: '0 24px' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="glass" style={{
            padding: '64px 48px', textAlign: 'center', borderRadius: 28,
            background: 'linear-gradient(135deg, rgba(14,165,233,0.06), rgba(56,189,248,0.04)), rgba(255,255,255,0.65)',
          }}>
          <h2 style={{ fontSize: 36, fontWeight: 800, color: '#0c1445', marginBottom: 16 }}>Help Us Improve Mira-Bhayandar</h2>
      ...
      <footer style={{
        ...
        borderTop: '1px solid rgba(255,255,255,0.8)', padding: '24px 48px',
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {['Home', 'Dashboard', 'Track', 'Report', 'Survey'].map(l => (
            <button key={l} onClick={() => onNavigate(l.toLowerCase())} style={{
              background: 'none', border: 'none', fontSize: 13, color: '#64748b', cursor: 'pointer', fontFamily: 'Inter, sans-serif',
            }}>{l}</button>
          ))}
        </div>
